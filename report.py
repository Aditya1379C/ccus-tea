...
<style>
  :root {
    ...
    --glass-sidebar: rgba(255,255,255,.7);
  ...
  html[data-theme="dark"] {
    ...
    --sidebar-bg: #11161f;
    --info-bg:   #0f141d;
    --glass-bg:  rgba(26,33,44,.45);
    --glass-bd:  rgba(255,255,255,.10);
    --glass-strong: rgba(22,29,40,.82);
    --glass-sidebar: rgba(17,22,31,.72);
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* slim scrollbar with a transparent gutter so the glass shows through */
  ...
  /* Solvent tabs */
  /* glass segmented control (matches the page switcher) */
  .solvent-tabs {
    display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 4px; padding: 4px;
    background: var(--glass-bg);
    -webkit-backdrop-filter: blur(16px) saturate(1.5);
    backdrop-filter: blur(16px) saturate(1.5);
    border: 1px solid var(--glass-bd); border-radius: 11px; box-shadow: var(--shadow);
  }
  .stab {
  ...
  .stab.sel-mea   { color: var(--mea);   background: color-mix(in srgb, var(--mea)   18%, transparent); box-shadow: 0 1px 3px rgba(0,0,0,.08); }
  .stab.sel-pz    { color: var(--pz);    background: color-mix(in srgb, var(--pz)    18%, transparent); box-shadow: 0 1px 3px rgba(0,0,0,.08); }
  .stab.sel-k2co3 { color: var(--k2co3); background: color-mix(in srgb, var(--k2co3) 18%, transparent); box-shadow: 0 1px 3px rgba(0,0,0,.08); }
  .solvent-info {
  ...
  input[type=range]::-webkit-slider-thumb {
    ...
    border-radius: 50%; background: var(--glass-strong);
    border: 1px solid var(--glass-bd);
    -webkit-backdrop-filter: blur(8px) saturate(1.5);
    backdrop-filter: blur(8px) saturate(1.5);
    box-shadow: 0 1px 3px rgba(0,0,0,.25), 0 0 0 3px var(--accent-lt);
    cursor: pointer;
  }
  ...
  /* subtle per-metric tint — a hue-true translucent color layered OVER the glass
     (mixing into the bluish glass-bg skews hues in dark mode) */
  .kpi.tint-blue   { background: linear-gradient(color-mix(in srgb, var(--accent)     15%, transparent), color-mix(in srgb, var(--accent)     15%, transparent)), var(--glass-bg); }
  .kpi.tint-indigo { background: linear-gradient(color-mix(in srgb, var(--kpi-capex) 15%, transparent), color-mix(in srgb, var(--kpi-capex) 15%, transparent)), var(--glass-bg); }
  .kpi.tint-green  { background: linear-gradient(color-mix(in srgb, var(--green)      15%, transparent), color-mix(in srgb, var(--green)      15%, transparent)), var(--glass-bg); }
  .kpi.tint-slate  { background: linear-gradient(color-mix(in srgb, #94a3b8           18%, transparent), color-mix(in srgb, #94a3b8           18%, transparent)), var(--glass-bg); }
  ...
  .kpi-value.indigo  { color: var(--kpi-capex); }
  ...
  .modal {
    ...
    /* fallback: plain frosted glass */
    -webkit-backdrop-filter: blur(28px) saturate(1.7);
    backdrop-filter: blur(28px) saturate(1.7);
    /* enhancement: refraction via SVG displacement (Chrome) */
    backdrop-filter: blur(7px) saturate(1.8) url(#liquid-glass);
    border: 1px solid var(--glass-bd);
    border-radius: 18px; padding: 26px 28px; max-width: 460px; width: 90%;
  ...
  .page-nav {
    display:inline-flex; gap:4px; margin-bottom:22px; padding:4px;
    align-self:flex-start;
    background:var(--glass-bg);
    -webkit-backdrop-filter:blur(16px) saturate(1.5);
    backdrop-filter:blur(16px) saturate(1.5);
    border:1px solid var(--glass-bd); border-radius:12px; box-shadow:var(--shadow);
  }
  #page-analysis, #page-overview { display:flex; flex-direction:column; gap:22px; }
  .page-tab {
    padding:7px 18px; font-size:12.5px; font-weight:600; font-family:inherit;
    border:none; background:transparent; color:var(--muted); cursor:pointer;
    border-radius:9px; transition:color .2s, background .2s, box-shadow .2s;
  }
...
<!-- Liquid-glass edge refraction (Chrome-only; squircle displacement map generated at runtime) -->
<svg width="0" height="0" style="position:absolute" aria-hidden="true">
  <filter id="liquid-glass" x="0" y="0" width="100%" height="100%" color-interpolation-filters="sRGB">
    <feImage id="lg-map" result="dmap" preserveAspectRatio="none"></feImage>
    <feDisplacementMap in="SourceGraphic" in2="dmap" scale="40" xChannelSelector="R" yChannelSelector="G"></feDisplacementMap>
  </filter>
</svg>
...
<main class="main">
  ...
  <!-- KPIs -->
  <div class="kpi-row">
    <div class="kpi is-primary tint-blue">
...
function fillSlider(id){
  ...
  const rest=cs.getPropertyValue('--track').trim();
...
// Capture the rect of the control that was clicked, so the panel can morph out of it (FLIP)
let sourceRect = null;
document.addEventListener("pointerdown", e => {
  const t = e.target.closest(".info-btn, .pfd-equip, .kpi, .stab");
  sourceRect = (t || e.target).getBoundingClientRect();
}, true);

const SPRING = "cubic-bezier(.34,1.56,.64,1)";   // overshoot-and-settle
let modalAnim = null, overlayAnim = null, lastMorph = null;

// ── Liquid-glass edge-refraction map (squircle bezel → R/G displacement) ──────
function buildDisplacementMap(w, h, radius, bezel) {
  w = Math.max(2, Math.round(w)); h = Math.max(2, Math.round(h));
  const c = document.createElement("canvas"); c.width = w; c.height = h;
  const ctx = c.getContext("2d");
  const img = ctx.createImageData(w, h); const d = img.data;
  const squircle = x => Math.pow(1 - Math.pow(1 - x, 4), 0.25);   // soft flat→curve

  // signed distance into a rounded rectangle (>=0 inside, = distance from edge)
  const halfW = w/2, halfH = h/2;
  function dist(px, py) {
    // depth inside a rounded rect (>0 inside, distance from the nearest edge)
    const qx = Math.abs(px - halfW) - halfW + radius;
    const qy = Math.abs(py - halfH) - halfH + radius;
    const ax = Math.max(qx, 0), ay = Math.max(qy, 0);
    const sdf = Math.min(Math.max(qx, qy), 0) + Math.hypot(ax, ay) - radius;
    return -sdf;
  }
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      let mag = 0, nx = 0, ny = 0;
      if (dd >= 0 && dd < bezel) {
        const t = dd / bezel;                                     // 0 at rim, 1 inner
        mag = Math.max(0, (squircle(Math.min(1,t+0.01)) - squircle(Math.max(0,t-0.01))) / 0.02);
        // inward normal = gradient of the distance field
        const gx = dist(px+1, py) - dist(px-1, py);
        const gy = dist(px, py+1) - dist(px, py-1);
  ...
  for (let i = 0; i < w*h; i++) {
    d[i*4]   = 128 + (vx[i]/maxMag) * 127;   // R → X displacement
    d[i*4+1] = 128 + (vy[i]/maxMag) * 127;   // G → Y displacement
...
function applyModalGlass(w, h) {
  const radius = 18, bezel = Math.min(26, Math.min(w, h) / 2.2);
  const fe = document.getElementById("lg-map");
  fe.setAttribute("href", buildDisplacementMap(w, h, radius, bezel));
  fe.setAttribute("x", 0); fe.setAttribute("y", 0);
  fe.setAttribute("width", Math.round(w)); fe.setAttribute("height", Math.round(h));
}

// Transform that maps the modal box onto the source control's box
function morphFrom(modalRect, src) {
  const s  = Math.max(0.05, src.width / modalRect.width);   // uniform scale from source width
  const tx = (src.left + src.width/2)  - (modalRect.left + modalRect.width/2);
  const ty = (src.top  + src.height/2) - (modalRect.top  + modalRect.height/2);
...
function showInfo(key) {
  ...
  applyModalGlass(modalRect.width, modalRect.height);   // refraction map sized to the panel

  if (modalAnim)   modalAnim.cancel();
  if (overlayAnim) overlayAnim.cancel();
  ...
  // Panel: consistent gentle rise + settle, identical from every trigger
  modalAnim = modal.animate([
...
function hideInfo() {
  ...
  const out = modal.animate([
    { transform: "scale(1) translateY(0)",     opacity: 1 },
    { transform: "scale(.97) translateY(6px)", opacity: 0 }
  ], { duration: 160, easing: "ease-in", fill: "forwards" });

  out.onfinish = () => { overlay.classList.remove("open"); };
}
